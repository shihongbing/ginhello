...
var(
	...
	SignKey          string = "ultrapower.ueba.api" // 签名信息应该设置成动态从库中获取
)

func JWTAuth() gin.HandlerFunc  {
	return func(context *gin.Context) {
		var result = model.NewResult(context)
		token := context.Request.Header.Get("access_token")
		//token为空
		if token == ""{
			result.Fail(constant.FailCode,"请求未携带token，无权限访问")
			context.Abort()
			return
		}
		logger.Logger.Info(context, "get token:", token)
		j := NewJWT()
		// 解析token中包含的相关信息
		...
		if err != nil {
			// token过期
			if err == TokenExpired {
				result.Fail(constant.FailCode,"token授权已过期，请重新申请授权")
				context.Abort()
				return
			}
			// 其他错误
